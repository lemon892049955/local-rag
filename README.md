...
| **v0.5** | **2026-04-08** | **智能助手 (AI 悬浮窗 + 意图识别 + 多轮对话 + 主动推送 + 企微对话升级)** ← 当前 |
...
- [x] **v0.5** — 智能助手 ← 当前版本
  - [x] 右下角悬浮窗 AI 助手（多轮对话 + 意图识别 + 快捷操作）
  - [x] 主动推送系统（每周摘要 / 知识回顾 / 关联推荐 / 编译通知）
  - [x] 对话中直接入库（粘贴链接自动识别）
  - [x] 定时任务框架（asyncio 后台调度）
  - [x] 双渠道推送（悬浮窗 + 企微）
- [ ] **v0.6** — 入库场景优化
  - 入库流程精细化（手动文本入库 / 批量 URL / 微信聊天记录）
  - 智能分类体系升级（自动分类 + 自定义分类规则 + 标签层级化）
  - 入库质量检测（重复检测增强 / 低质量过滤 / 内容去重）
  - 来源归一化（同一事件多来源合并 / 来源可信度标注）
  - 入库后处理流程（摘要质量评估 / 标签推荐 / 分类纠偏）
- [ ] **v1.0** — 成熟版
  - 稳定运行 3 个月+
  - 多端访问 (PWA)
  - 定时备份到对象存储
