...
| **v2.4** | **2026-04-18** | **微信 iLink Bot 接入（替代企微自建应用）+ 强制重入库 + 搜索历史** |
| v2.3 | 2026-04-15 | Reranker 模型从 HF 镜像加载 + 入库报错修复 |
| v2.2 | 2026-04-10 | 混合检索优化（意图路由 + 查询改写 + RRF 融合 + Reranker） |
| v2.1 | 2026-04-09 | Wiki 分类体系（Taxonomy）+ 概念订阅 |
| v2.0 | 2026-04-08 | 架构重构：双层检索（Wiki+Data）+ SSE 流式 + 前端升级 |
| v0.6.5 | 2026-04-08 | 前端重构 (图谱标签聚合 + Markdown 美化 + 编辑功能) |
...
- [x] **v2.0** — 架构重构（双层检索 + SSE 流式 + 前端升级）
- [x] **v2.1** — Wiki 分类体系（Taxonomy）+ 概念订阅
- [x] **v2.2** — 混合检索优化（意图路由 + 查询改写 + RRF + Reranker）
- [x] **v2.3** — Reranker HF 镜像加载 + 入库修复
- [x] **v2.4** — 微信 iLink Bot 接入 + 强制重入库 + 搜索历史 ← 当前版本
- [ ] **v0.7** — RAG 策略优化
